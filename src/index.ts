...
const TYPE_DEFINITION_KINDS = new Set(['class', 'struct', 'union', 'interface', 'enum', 'concept']);

function definitionSectionTitle(compounds: Compound[]): string {
  return compounds.every((compound) => TYPE_DEFINITION_KINDS.has(compound.kind))
    ? 'Class Definitions'
    : 'Definitions';
}

function renderCompoundPage(primary: Compound, definitions: Compound[] = []): (string | undefined)[] {
  const contents: (string | undefined)[] = [
    templates.render(primary, { headingBase: 1 }),
  ];

  if (definitions.length) {
    contents.push(`\n## ${definitionSectionTitle(definitions)}\n\n`);
    contents.push(...definitions.map((compound) => templates.render(compound, { headingBase: 3 })));
  }
...
export async function run(options: Partial<MoxygenOptions> & { directory: string }): Promise<void> {
  ...
  if (splitOutput) {
    if (writeRootIndex) {
      const contents = renderCompoundPage(root, rootBodyCompounds);
    ...
    if (opts.groups) {
      for (const group of groups) {
        ...
        writeWithOptionalFrontmatter(group, renderCompoundPage(group, compounds), references, opts, anchorMap, pagePathMap);
      }
    }

    if (opts.classes) {
      for (const comp of classOutputCompounds) {
        writeWithOptionalFrontmatter(comp, [templates.render(comp, { headingBase: 1 })], references, opts, anchorMap, pagePathMap);
      }
    }
  } else {
